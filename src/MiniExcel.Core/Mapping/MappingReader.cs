...
internal static partial class MappingReader<T> where T : class, new()
{
    ...
    public static async Task<IEnumerable<T>> QueryAsync(Stream stream, CompiledMapping<T> mapping, CancellationToken cancellationToken = default)
    {
        ...
        // Path for non-optimized mappings
    ...
    {
        return data
            .SelectMany((row, index) => row.Select(kvp => (Address: $"{kvp.Key}{index + 1}", Value: kvp.Value)))
    ...
    private static IEnumerable<T> ReadMappedData(Dictionary<string, object?> cellLookup, CompiledMapping<T> mapping)
    {
        ...
        var maxPropertyRow = Math.Max(0, mapping.Properties.Max(prop => prop.CellRow));

        // Also check collection start rows as they may be part of the item definition
        maxPropertyRow = Math.Max(maxPropertyRow, mapping.Collections.Max(coll => coll.StartCellRow));
        ...
        var baseRow = Math.Min(int.MaxValue, mapping.Properties.Min(prop => prop.CellRow));
        ...
        while (itemsFound < maxItems)
        {
            ...
            for (int collIndex = 0; collIndex < mapping.Collections.Count; collIndex++)
            {
                ...
                if (maxRow is null && coll.Layout == CollectionLayout.Vertical)
                {
                    ...
                    if (mapping.Properties.Any())
                    {
                        ...
                        foreach (var prop in mapping.Properties)
                        {
                            if (ReferenceHelper.ParseReference(prop.CellAddress, out int propCol, out int propRow))
                            {
                                ...
                                if (cellLookup.TryGetValue(nextItemCell, out var value) && value is not null)
                                {
    ...
    private static List<object> ReadCollectionDataWithOffset(Dictionary<string, object> cellLookup, CompiledCollectionMapping coll, string offsetStartCell, int? maxRow = null, int? maxCol = null)
    {
        ...
        {
            if (coll is { ItemMapping: not null, ItemType: not null })
            {
            ...
            else
            {
                ...
                if (cellLookup.TryGetValue(cellAddress, out var value) && value is not null && !string.IsNullOrEmpty(value.ToString()))
                {
                    results.Add(value);
                    emptyCellCount = 0;
                }
    ...
    {
        if (coll.ItemType is null || coll.ItemMapping is null)
            return null;
            
        if(Activator.CreateInstance(coll.ItemType) is not { } item)
            return null;
        ...
        if (propsProperty?.GetValue(itemMapping) is not IEnumerable<CompiledPropertyMapping> properties) 
            return null;
    ...
    {
        // Only vertical layout is supported
        return ReferenceHelper.ParseReference(baseCellAddress, out int baseColumn, out int baseRow) 
            ? ReferenceHelper.ConvertCoordinatesToCell(baseColumn, currentRow) 
            : baseCellAddress;
    }
    ...
    private static object? ConvertToTypedCollection(List<object> items, Type collectionType, Type? itemType)
    {
        ...
        if (collectionType.IsGenericType && collectionType.GetGenericTypeDefinition() == typeof(List<>))
        {
            ...
            {
                if (ConversionHelper.ConvertValue(item, elementType) is { } convertedValue)
                {
        ...
        if (collectionType.IsGenericType && 
            (collectionType.GetGenericTypeDefinition() == typeof(IEnumerable<>) ||
             collectionType.GetInterface(typeof(IEnumerable<>).Name) is not null))
        {
            ...
            if (isColumnLayout)
            {
                ...
                await foreach (var row in reader.QueryAsync(false, mapping.WorksheetName, "A1", cancellationToken).ConfigureAwait(false))
                {
                    ...
                    foreach (var prop in mapping.Properties)
                    {
                        ...
                        if (!row.TryGetValue(columnLetter, out var value) || value is null || prop.Setter is null)
                            continue;
                ...
                if (handler.CollectionIndex >= 0 && collections.ContainsKey(handler.CollectionIndex))
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if (handler.CollectionItemConverter is not null)
                            {
        ...
        object value, Type itemType, object nestedMapping)
    {
        ...
        if (collection[handler.CollectionItemOffset] is not { } item)
        {
        ...
        if (handler.ValueSetter is null)
        {
    ...
    private static void FinalizeCollections(T item, CompiledMapping<T> mapping, Dictionary<int, IList> collections)
    {
        for (int i = 0; i < mapping.Collections.Count; i++)
        {
            ...
            {
                var lastItem = list[^1];
                var isDefault = lastItem is null || 
            ...
            if (collectionMapping.Setter is not null)
            {
                ...
                if (propInfo is not null && propInfo.PropertyType.IsArray)
                {
    ...
    private static bool HasAnyData(T item, CompiledMapping<T> mapping)
    {
        ...
        if (mapping.Properties.Any(prop => prop.Getter(item) is { } value && !IsDefaultValue(value)))
            return true;

        // Check if any collections have items
        if(mapping.Collections.Any(coll => coll.Getter(item) is { } collection && collection.Cast<object>().Any()))
            return true;
    ...
    private static bool TryParseRowFromCellReference(string cellRef, out int row)
    {
        ...
        if (i < cellRef.Length && int.TryParse(cellRef[i..], out row))
        {
            return row > 0;
        }
